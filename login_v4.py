...
def run_login_on_row(
    ...
    try:
        _execute_login_logic(driver, row_dict, df, row_index, target_ddmm)
...
def _execute_login_logic(driver, row_dict, df, index, target_date):
    """Original login logic preserved exactly as requested"""
    try:
        if not _perform_login_step(driver, row_dict, df, index):
            return
        _perform_reservation_step(driver, row_dict, df, index, target_date)
    except Exception as e:
        print(f"❌ Erreur : {e}")
...
def _perform_login_step(driver, row_dict, df, index):
    ...
    nationalite = row_dict["nationalite"]
    ...
    numero_passport = row_dict["numero_passport"]
    ...
    # Récupérer le code
    code = get_verification_code(row_dict['email'])
    if code:
        ...
        if otp_errors:
            ...
            if any(k in error_msg for k in ["invalid", "incorrect", "wrong", "expired", "otp", "code"]):
                ...
                if otp_error_count < max_otp_retries:
                    ...
                    time.sleep(1)
                    new_code = get_verification_code(row_dict['email'])
...
def _perform_reservation_step(driver, row_dict, df, index, target_date):
    wait = WebDriverWait(driver, 10, poll_frequency=0.2)
    ...
    try:
        WebDriverWait(driver, 8, 0.2).until(
            EC.visibility_of_element_located((AppiumBy.ID, "com.moh.nusukapp:id/tvTime"))
        )
    except Exception:
